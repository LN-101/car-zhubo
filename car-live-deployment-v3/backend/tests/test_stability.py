...
def test_unitized_stream_request_preserves_frontend_phrase(monkeypatch):
    monkeypatch.setattr(settings, 'tts_provider', 'idextts2')
...
def test_zhubo_untagged_unitized_request_keeps_phrase(monkeypatch):
    monkeypatch.setattr(settings, 'tts_provider', 'idextts2')
    request = main.TTSRequest(text="第一句话，介绍今天的直播主题。", unitized=True)
    assert _tts_stream_units(request) == [normalize_tts_text(request.text)]


def test_zhubo_live_item_uses_short_phrase_units(monkeypatch):
    monkeypatch.setattr(settings, 'tts_provider', 'zhubo')
    text = "老板，今天为大家介绍欧拉 5 EV 2026 款 580km 激光雷达版。"
    normalized = normalize_tts_text(text)
    units = _tts_stream_units(main.TTSRequest(text=text, unitized=True, item_id=1))
    assert ''.join(units) == normalized
    # zhubo-style short phrases: the first two segments stay within the first
    # limit and later ones within the follow-up limit, so audio starts after the
    # first phrase instead of the whole sentence.
    assert len(units) >= 2
    # The first segment stays within the first limit; later segments stay within
    # the follow-up limit (an unbreakable single phrase may still be longer).
    assert len(units[0]) <= 16
    assert all(len(unit) <= 32 for unit in units)
