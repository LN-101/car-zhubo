"""IndexTTS-2/2.5 adapter with enhanced parameter control.
...
Enhanced with tone/emotion control from zhubo/tts_broadcast.
"""
...
# Tone/emotion presets from zhubo/tts_broadcast
TONES = {
    "neutral": None,
...
class SynthesisParameters:
    """Enhanced TTS parameters with tone/emotion control."""
    speed: float = 1.0
    volume: float = 100.0
    tone: str = "neutral"
    intensity: float = 0.3  # Default emotion blend for natural delivery

    def __post_init__(self):
        if not (0.5 <= self.speed <= 2.0 and 0 <= self.volume <= 200
                and 0 <= self.intensity <= 1.0 and self.tone in TONES):
            raise ValueError("参数超出范围")

    def duration_factor(self) -> float:
        """Convert speed to IndexTTS duration_factor (inverse)."""
        return 1.0 / self.speed

    def emo_vector(self) -> list[float] | None:
        """Get emotion vector scaled by intensity."""
        vector = TONES[self.tone]
        if vector is None:
            return None
        return [v * self.intensity for v in vector]
