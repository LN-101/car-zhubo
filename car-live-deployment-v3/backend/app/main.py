...
def _tts_provider():
    ...
    if value in {"idextts2", "index-tts2", "indextts2", "index-tts-2"}:
        return "idextts2"
    if value in {"zhubo", "zhubo-tts"}:
        return "zhubo"
    return "gpt-sovits"


# Initialize Zhubo TTS Adapter lazily
ZHUBO_ADAPTER = None

def _get_zhubo_adapter():
    """Get or initialize the Zhubo TTS adapter."""
    global ZHUBO_ADAPTER
    if ZHUBO_ADAPTER is None and _tts_provider() == "zhubo":
        from .tts.adapters.zhubo_adapter import ZhuboTTSAdapter
        try:
            ZHUBO_ADAPTER = ZhuboTTSAdapter(base_url=settings.zhubo_tts_url)
            logger.info(f"Initialized Zhubo TTS adapter at {settings.zhubo_tts_url}")
        except Exception as e:
            logger.error(f"Failed to initialize Zhubo TTS adapter: {e}")
    return ZHUBO_ADAPTER
...
class Progress(BaseModel):
    # The browser reports the item it is currently playing so the backend can
    # keep 待播/播放中/已播放 in sync and protect finished items from edits.
    current_item_id: int | None = Field(default=None, ge=1)
    status: str = Field(default="playing", max_length=40)
...
class TTSRequest(BaseModel):
    ...
    # queueing rules; a unitized request that also sets `stream_batch` must be
    # preserved as one unit, while a unitized request without it still goes
    # through the natural-punctuation splitter.
    ...
    # TTS Broadcast parameters (from zhubo/tts_broadcast)
    tone: str = Field(default="neutral", pattern='^(neutral|warm|energetic|steady)$')
    intensity: float = Field(default=0.3, ge=0.0, le=1.0)
    volume: float = Field(default=1.0, ge=0.0, le=2.0)
    # Live-session item identity. The browser tags each live request with the
    # item id and revision it belongs to so stale audio can be discarded; the
    # synthesis path itself is stateless.
    item_id: int | None = Field(default=None, ge=1)
    item_revision: int | None = Field(default=None, ge=0)
...
def config():
    ...
    elif active_provider == "zhubo":
        adapter = _get_zhubo_adapter()
        tts = {"mode": "zhubo", "provider": "Zhubo TTS", "ready": bool(adapter and adapter.health_check())}
    else:
...
def tts_status():
    ...
    if _tts_provider() == "zhubo":
        adapter = _get_zhubo_adapter()
        reachable = bool(adapter and adapter.health_check())
        voices = []
        if reachable:
            try:
        ...
        return {
            "provider": "zhubo",
            "provider_label": "Zhubo TTS",
            "ready": reachable,
            "configured": True,
            "reachable": reachable,
            "cloned_voices": len(voices),
            "endpoint": settings.zhubo_tts_url,
            "model_version": "zhubo",
            "streaming": reachable,
            "stream_mode": "pcm-wav-natural",
            "streaming_mode": 1,
            "live_streaming_mode": 2,
            "active_model_profile": "zhubo",
            "model_profiles": ["zhubo"],
            "warming_up": False,
            "calibrating_voice_ids": [],
            "warmed_voice_ids": [],
        }
...
def _normalize_reference_audio(source: Path, *, force: bool = True):
    ...
    filter_graph = (
        "silenceremove=start_periods=1:start_duration=0.30:start_silence=0.30:start_threshold=-50dB,"
        "areverse,silenceremove=start_periods=1:start_duration=0.80:start_silence=0.80:start_threshold=-50dB,areverse"
    )
...
def _warm_single_voice(voice_id: str):
    ...
    # Zhubo owns its model and reference audio; never route clone validation through GPT-SoVITS.
    if _tts_provider() == "zhubo":
        VOICE_WARMING.add(voice_id)
        try:
...
def _voice_config(voice_id: str):
    ...
    if _tts_provider() == "zhubo":
        ...
        if not row["reference_path"]:
            raise HTTPException(422, "该音色没有参考音频")
        quality = _audio_quality(row["reference_path"])
        if quality.get("status") != "ready":
            raise HTTPException(422, f"该音色参考音频不可用：{quality.get('message', '缺失或无效')}")
        return row["reference_path"], row["prompt_text"] or "", row["prompt_lang"] or "zh"
    global_reference = _tts_global_reference_audio()
...
def _zhubo_script_units(text: str, first_limit: int = 16, limit: int = 32) -> list[str]:
    """Broadcast segmentation in the style of zhubo's tts_broadcast.split_script.

    Split at natural punctuation and group the fragments so the first two
    segments stay within ``first_limit`` characters and later ones within
    ``limit``. This keeps each live unit short (a phrase, not a whole sentence),
    so audio starts after the first phrase instead of the complete sentence.
    """
    normalized = normalize_tts_text(text)
    phrases = [
        phrase.strip()
...
def _tts_stream_units(req: TTSRequest):
    """Resolve stream units once.

    A unitized live batch (``unitized && stream_batch``) is preserved intact
    because the browser owns those queue boundaries. A unitized request without
    ``stream_batch`` goes through the broadcast segmentation: the zhubo live
    path uses zhubo-style short phrases, other engines keep the existing
    natural-punctuation splitter. Direct API callers can omit ``unitized`` and
    retain the server-side fallback splitter.
    """
    ...
    if req.unitized:
        normalized = normalize_tts_text(req.text)
        if _tts_provider() == "zhubo":
            return _zhubo_script_units(normalized)
...
def _ensure_tts_ready():
    if _tts_provider() == "zhubo":
        adapter = _get_zhubo_adapter()
        if adapter is None:
            raise HTTPException(503, "Zhubo TTS 适配器未初始化")
        if not adapter.health_check():
            raise HTTPException(503, "Zhubo TTS 服务未启动或不可访问")
        return
    if _tts_provider() == "idextts2":
...
def synthesize_tts(req: TTSRequest):
    ...
    if provider == "zhubo":
        ...
        try:
            adapter = _get_zhubo_adapter()
            if adapter is None:
                raise HTTPException(502, "Zhubo TTS 适配器未初始化")
            ...
            raise HTTPException(502, f"Zhubo TTS 服务调用失败：{exc}") from exc
    ...
    if provider == "zhubo":
        ...
        async def zhubo_iterator():
            ...
            try:
                adapter = _get_zhubo_adapter()
                if adapter is None:
                    raise RuntimeError("Zhubo TTS 适配器未初始化")
                ...
                raise RuntimeError(f"Zhubo TTS 流式调用失败：{exc}") from exc
...
def create_session(s: Session):
    sid = uuid4().hex
    texts = _session_script_items(s.script)
    with conn() as c:
        ...
        for position, text in enumerate(texts):
            c.execute(
                "INSERT INTO session_items(session_id,item_id,position,text,revision,played) VALUES(?,?,?,?,0,0)",
                (sid, position + 1, position, text),
            )
    return session(sid)


def _session_script_items(text: str) -> list[str]:
    """Split the initial script into live items.

    The backend owns the item list, and the items use the same segmentation as
    playback, so the 实时话术 编号/状态 rows show exactly the short phrases that
    the broadcast plays: the zhubo live path uses the zhubo-style short-phrase
    splitter, other engines keep the sentence-level splitter.
    """
    normalized = normalize_tts_text(text or "")
    if not normalized:
        return []
    if _tts_provider() == "zhubo":
        return _zhubo_script_units(normalized)
...
def _session_payload(c, row):
    current = row["current_item"]
    rows = c.execute(
        "SELECT item_id,text,revision,played FROM session_items WHERE session_id=? ORDER BY position",
        (row["id"],),
    ).fetchall()
    data = dict(row)
    data["items"] = [
        {
            ...
            "status": "已播放" if item["played"] else (
                "播放中" if current is not None and item["item_id"] == current else "待播"
            ),
        }
        for item in rows
    ]
...
def session(sid):
    with conn() as c:
        return _session_payload(c, _require_session(c, sid))


def _require_editable_item(c, sid, item_id):
    item = c.execute(
        "SELECT item_id,position,text,revision,played FROM session_items WHERE session_id=? AND item_id=?",
        (sid, int(item_id)),
    ).fetchone()
    if not item:
        raise HTTPException(404, "话术条目不存在")
    if item["played"]:
        raise HTTPException(409, "该条目已播报，不能修改或删除。请在后续待播条目中修改，或新增话术。")
    return item
...
@app.patch("/api/live/sessions/{sid}/items/{item_id}")
def edit_item(sid: str, item_id: int, r: ItemEdit):
    text = normalize_tts_text(r.text).strip() or r.text.strip()
    if not text:
        raise HTTPException(422, "话术不能为空；移除话术请使用删除")
    with conn() as c:
        _require_session(c, sid)
        _require_editable_item(c, sid, item_id)
        c.execute(
            "UPDATE session_items SET text=?,revision=revision+1 WHERE session_id=? AND item_id=?",
            (text, sid, int(item_id)),
        )
        c.execute("UPDATE sessions SET version=version+1,updated_at=? WHERE id=?", (now(), sid))
        return _session_payload(c, _require_session(c, sid))


@app.post("/api/live/sessions/{sid}/items")
def add_item(sid: str, a: ItemAdd):
    text = normalize_tts_text(a.text).strip() or a.text.strip()
    if not text:
        raise HTTPException(422, "话术不能为空")
    with conn() as c:
        session_row = _require_session(c, sid)
        rows = c.execute(
            "SELECT item_id,position,played FROM session_items WHERE session_id=? ORDER BY position",
            (sid,),
        ).fetchall()
        new_id = max((row["item_id"] for row in rows), default=0) + 1
        # The new item must not land in the already-played prefix, and it must
        # not appear before the item the browser is currently playing: the
        # console plays it right after the audible item, so the 编号 table and
        # the audible order stay in step.
        floor_position = sum(1 for row in rows if row["played"])
        if session_row["current_item"] is not None:
            playing = next((entry for entry in rows if entry["item_id"] == session_row["current_item"]), None)
            if playing is not None:
                floor_position = max(floor_position, playing["position"] + 1)
        if a.after_id is None:
            position = len(rows)
        else:
            target = next((row for row in rows if row["item_id"] == a.after_id), None)
            if target is None:
                raise HTTPException(404, "目标话术编号不存在")
            position = max(target["position"] + 1, floor_position)
        c.execute(
            "UPDATE session_items SET position=position+1 WHERE session_id=? AND position>=?",
            (sid, position),
        )
        c.execute(
            "INSERT INTO session_items(session_id,item_id,position,text,revision,played) VALUES(?,?,?,?,0,0)",
            (sid, new_id, position, text),
        )
        c.execute("UPDATE sessions SET version=version+1,updated_at=? WHERE id=?", (now(), sid))
        return _session_payload(c, _require_session(c, sid))


@app.delete("/api/live/sessions/{sid}/items/{item_id}")
def delete_item(sid: str, item_id: int):
    with conn() as c:
        row = _require_session(c, sid)
        item = _require_editable_item(c, sid, item_id)
        c.execute("DELETE FROM session_items WHERE session_id=? AND item_id=?", (sid, int(item_id)))
        c.execute(
            "UPDATE session_items SET position=position-1 WHERE session_id=? AND position>?",
            (sid, item["position"]),
        )
        if row["current_item"] == int(item_id):
            c.execute("UPDATE sessions SET current_item=NULL WHERE id=?", (sid,))
        c.execute("UPDATE sessions SET version=version+1,updated_at=? WHERE id=?", (now(), sid))
        return _session_payload(c, _require_session(c, sid))


@app.patch("/api/live/sessions/{sid}/progress")
def progress(sid: str, p: Progress):
    with conn() as c:
        _require_session(c, sid)
        if p.current_item_id is None:
            c.execute("UPDATE session_items SET played=1 WHERE session_id=?", (sid,))
            c.execute(
                "UPDATE sessions SET current_item=NULL,status=?,updated_at=? WHERE id=?",
                (p.status, now(), sid),
            )
        else:
            item = c.execute(
                "SELECT position FROM session_items WHERE session_id=? AND item_id=?",
                (sid, int(p.current_item_id)),
            ).fetchone()
            if not item:
                raise HTTPException(404, "话术条目不存在")
            c.execute(
                "UPDATE session_items SET played=1 WHERE session_id=? AND position<?",
                (sid, item["position"]),
            )
            c.execute(
                "UPDATE sessions SET current_item=?,status=?,updated_at=? WHERE id=?",
                (int(p.current_item_id), p.status, now(), sid),
            )
        return _session_payload(c, _require_session(c, sid))
...
def revise(sid: str, r: Revision):
    """Re-sync the whole script against the session item list.

    Played items are preserved as-is; the unplayed tail is replaced by the new
    script's tail, changing a queued item's text in place when it still lines
    up. A re-sync that would alter or drop a played item is rejected.
    """
    target = _session_script_items(r.script)
    with conn() as c:
        row = _require_session(c, sid)
        rows = c.execute(
            "SELECT item_id,position,text,played FROM session_items WHERE session_id=? ORDER BY position",
            (sid,),
        ).fetchall()
        played = [item for item in rows if item["played"]]
        pending = [item for item in rows if not item["played"]]
        played_texts = [item["text"] for item in played]
        if target[:len(played_texts)] != played_texts:
            raise HTTPException(409, "已播报的条目不能修改；请只修改后续待播条目")
        tail = target[len(played_texts):]
        next_id = max((item["item_id"] for item in rows), default=0) + 1
        for index, text in enumerate(tail):
            if index < len(pending):
                ...
                if item["text"] != text:
                    c.execute(
                        "UPDATE session_items SET text=?,revision=revision+1 WHERE session_id=? AND item_id=?",
                        (text, sid, item["item_id"]),
                    )
            else:
                c.execute(
                    "INSERT INTO session_items(session_id,item_id,position,text,revision,played) VALUES(?,?,?,?,0,0)",
                    (sid, next_id, len(played) + index, text),
                )
                next_id += 1
        for item in pending[len(tail):]:
            c.execute("DELETE FROM session_items WHERE session_id=? AND item_id=?", (sid, item["item_id"]))
        ordered = c.execute(
            "SELECT item_id FROM session_items WHERE session_id=? ORDER BY position",
            (sid,),
        ).fetchall()
        for position, item in enumerate(ordered):
            c.execute(
                "UPDATE session_items SET position=? WHERE session_id=? AND item_id=?",
                (position, sid, item["item_id"]),
            )
        c.execute(
            "UPDATE sessions SET script=?,version=version+1,sentence=?,updated_at=? WHERE id=?",
            (r.script, r.sentence, now(), sid),
        )
        return _session_payload(c, _require_session(c, sid))
