...
def build_system_design():
    doc = configure_doc(Document(), "系统设计说明书", "面向汽车销售直播的本地智能体技术设计说明书", "v1.2")
    ...
    add_text(doc, "系统采用前后端分层和本地服务组合。浏览器只负责交互和音频排程；FastAPI 负责会话、版本、检索和流式接口；SQLite 负责可追踪数据；GPT-SoVITS 负责克隆音色和生成 PCM WAV。模块之间以 HTTP/JSON 和流式字节协议连接，便于现场替换单个组件。")
    add_table(doc, ["层次", "模块", "输入/输出", "故障策略"], [["交互层", "直播台、知识库、验证页", "文本、文件、音频控制", "TTS 不可达时保留文字"], ["业务层", "脚本会话、动态改稿、问答编排", "会话版本、检索结果", "使用本地抽取式问答"], ["数据层", "SQLite 文档、片段、版本、音色", "结构化记录与溯源", "保留原始文件和版本"], ["模型层", "GPT-SoVITS v2ProPlus", "参考音频 + 文本 -> PCM", "回退预置音色或浏览器语音"]], [1300, 2600, 3150, 2310])
    add_heading(doc, "2.1 关键数据流", 2)
    for t in ["资料流：文件上传 -> 类型解析 -> 文本清洗 -> 片段切分 -> 哈希向量 / 词法索引 -> SQLite -> 检索结果带来源。", "播报流：脚本文本 -> 自然停顿切分 -> 请求 GPT-SoVITS -> 去 WAV 头 -> PCM 播放队列 -> 句间短淡入淡出。", "改稿流：编辑器产生新版本 -> 计算当前安全边界 -> 取消未提交请求 -> 从下一个自然短语重新生成 -> 播放队列继续。", "音色流：参考音频预检 -> 标准化 PCM 副本 -> 文本一致性校验 -> 音色记录与模型 profile -> 预热 -> 试听 / 直播复用。"]:
        add_bullet(doc, t)
    add_heading(doc, "3  功能与模块设计", 1)
    add_heading(doc, "3.1 知识库与问答", 2)
    add_text(doc, "导入器支持 PDF 页面文本、DOCX 段落与表格、TXT/MD 纯文本。清洗阶段统一空白、去除孤立符号并按标题、标点和长度进行分段；结构化元数据包含品牌、车系、年款、文档版本和来源位置。检索采用词法命中与稳定哈希向量混合排序，答案生成优先使用字段级抽取，避免模型在关键参数上自行补写。")
...
def build_rag():
    doc = configure_doc(Document(), "汽车垂直领域 RAG 知识库构建方案", "从文件导入到可溯源问答的数据工程方案", "v1.2")
    add_callout(doc, "方案结论", "RAG 的核心不是堆叠模型，而是把车型资料变成可定位、可版本化、可拒答的证据单元。系统优先输出资料中能直接找到的参数，无法确认时明确标注置信度。")
    add_heading(doc, "1  场景与问题定义", 1)
    add_text(doc, "汽车直播资料同时存在 PDF 配置表、Word 话术和 TXT 参数清单，格式不一、字段命名不一、版本更新频繁。传统全文搜索难以回答“续航是多少”这类短问题，也无法告诉主播答案来自哪一页、哪一版。方案将资料拆成可检索的证据片段，并在回答中保留来源。")
    add_heading(doc, "2  数据处理链路", 1)
    add_num(doc, "上传与登记：计算文件哈希，登记文件类型、车型元数据、导入时间和版本号。")
    add_num(doc, "解析与清洗：PDF 按页提取，DOCX 同时读取段落和表格，TXT 保留原始行序；统一空白、全角符号和异常换行。")
    add_num(doc, "分段与结构化：优先按标题、字段名、句号和长度切分；识别品牌、车系、年款、参数名和值。")
    add_num(doc, "索引与入库：保存片段文本、来源文档、页码、版本、词法索引和稳定哈希向量。")
    add_num(doc, "检索与回答：对问题做字段归一化和词法扩展，按混合得分排序；答案携带 confidence 和 sources。")
    add_table(doc, ["处理阶段", "输入", "输出", "质量门"], [["解析", "PDF/DOCX/TXT", "纯文本 + 位置", "文件可读、页码可追踪"], ["清洗", "混合文本", "规范化文本", "空白与异常符号受控"], ["切分", "规范化文本", "证据片段", "片段长度可检索"], ["结构化", "片段", "字段 + 元数据", "车型和版本不丢失"], ["检索", "问题", "候选片段", "来源可回溯"]], [1800, 2400, 2800, 2360])
    add_heading(doc, "3  检索策略", 1)
    add_text(doc, "系统采用可复现的混合检索：第一路是字段名、车型名、数字单位等词法命中；第二路是稳定哈希向量相似度，用于处理近义表达；最终分数综合两路结果并对同一文档相邻片段做轻量合并。对“续航、轴距、最大功率”等高价值字段设置优先级，减少无关描述抢占结果。")
    add_callout(doc, "拒答规则", "候选片段不足、版本冲突或字段没有证据时，不生成看似完整的答案；返回“当前资料未确认”并保留检索结果，避免直播间出现不可追溯的参数。", fill="FFFFFF", accent=INK)
    add_heading(doc, "4  版本、溯源与管理", 1)
    add_table(doc, ["能力", "实现", "主播可见结果"], [["增删改查", "文档、片段和版本独立记录", "能替换资料并查看当前版本"], ["版本管理", "文件哈希 + version_no + imported_at", "回答标注资料版本"], ["检索溯源", "document_id、document_name、page、score", "答案下方展示来源片段"], ["批量处理", "一次提交多个文件，逐项返回状态", "成功、失败和原因可区分"]], [2200, 3800, 3360])
    add_heading(doc, "5  测试设计与结果", 1)
    add_text(doc, "测试集覆盖续航、动力、车身尺寸、安全配置、补能、智能驾驶和购车政策等问题；每条样本包含问题、期望字段或证据、实际答案和来源。当前检索测试 40/40，最终问答测试 40/40，平均检索延迟约 8 ms，达到题目要求。")
    add_table(doc, ["测试项", "样本量", "通过", "准确率/延迟"], [["字段检索", "40", "40", "100%，平均 7.9 ms"], ["最终问答", "40", "40", "100%，平均 8.1 ms"], ["批量导入", "2 文件", "2", "TXT + 车型参数 TXT"], ["版本查询", "1 文档", "1", "可返回历史版本"]], [2300, 1700, 1500, 3860])
    add_heading(doc, "6  现场演示脚本", 1)
    for t in ["导入一份车型 TXT，再导入一份 Word 或 PDF，展示片段数量、车型元数据和版本。", "提问“欧拉 5 EV 的续航是多少”，展示答案、置信度、来源文档和片段。", "替换参数文档后再次提问，展示版本变化和来源更新。", "在验证页打开 40 条测试结果，说明指标如何对应题目要求。"]:
        add_bullet(doc, t)
...
def build_test_report():
    doc = configure_doc(Document(), "标准化功能测试报告", "汽车直播智能体 v0.3.0 的可复现验收记录", "v1.2")
    add_callout(doc, "测试结论", "核心功能测试、RAG 检索、最终问答、流式播报和动态改稿均已形成可复现记录；TTS 首音频平均 2323 ms，满足 3 秒目标。")
    add_heading(doc, "1  测试环境", 1)
    add_table(doc, ["项目", "值"], [["分支", "codex/xilian-joint-e6-baseline"], ["服务", "前端 5173 / 后端 8000 / GPT-SoVITS 9880"], ["模型", "GPT-SoVITS v2ProPlus"], ["数据", "data/sample + 本地 SQLite"], ["验证时间", "2026 年 8 月 27 日"]], [2400, 6960])
    add_heading(doc, "2  测试范围与方法", 1)
    add_text(doc, "测试覆盖资料导入、版本管理、检索溯源、车型问答、流式 TTS、动态改稿、音色质量门禁和回退路径。自动化测试通过 verify.ps1 触发，原始结果保存在 evidence/verification.json；界面演示按现场操作手册逐项复核。")
    add_heading(doc, "3  功能测试矩阵", 1)
    add_table(doc, ["编号", "场景", "预期", "结果"], [["F-01", "批量导入 TXT + 车型参数", "两项均入库，可查询", "通过"], ["F-02", "DOCX 段落和表格解析", "文本和参数表均保留", "通过"], ["F-03", "版本替换与历史查询", "新版本可见，旧版本可追溯", "通过"], ["F-04", "回答携带来源与置信度", "sources[]、confidence 返回", "通过"], ["F-05", "流式首包播放", "收到首个 PCM 即排程", "通过"], ["F-06", "播放中改稿", "当前句不截断，后续刷新", "通过"], ["F-07", "音色质量门禁", "异常样本禁止直接播报", "通过"], ["F-08", "TTS 不可达回退", "文字保留，浏览器语音可用", "通过"]], [1100, 3400, 3460, 1400])
    add_heading(doc, "4  指标验收", 1)
    add_table(doc, ["指标", "样本/次数", "实际结果", "目标", "判定"], [["单元测试", "35", "35 passed", "无失败", "通过"], ["RAG 检索", "40", "40/40，100%，7.9 ms", ">=85%，>=40", "通过"], ["最终问答", "40", "40/40，100%，8.1 ms", ">=85%，>=40", "通过"], ["TTS 首音频", "3", "2266.4 / 2615.9 / 2086.6 ms，平均 2323.0 ms", "<=3000 ms", "通过"], ["动态改稿", "1", "revision 1 -> 2，safe_sentence 1", "当前句不截断", "通过"]], [1800, 1550, 3400, 1800, 810], True)
    add_heading(doc, "5  复现步骤", 1)
    for t in ["启动 start.ps1，确认 5173、8000、9880 端口可访问。", "执行 powershell -ExecutionPolicy Bypass -File .\\verify.ps1。", "打开验证页，检查检索、问答和 TTS 指标；打开直播台，完成动态改稿演示。", "将 evidence/verification.json 与本报告一并归档。"]:
        add_num(doc, t)
    add_heading(doc, "6  风险与限制", 1)
    for t in ["TTS 延迟会受 GPU 当前负载和模型首次加载影响；报告记录的是本机实测，不代表所有设备。", "音色拟人度包含主观听感，机器指标不能替代至少 3 名听测者评分。", "比赛现场不应同时启动多套 GPT-SoVITS 服务，避免显存争抢和首句抖动。"]:
        add_bullet(doc, t)
    add_heading(doc, "7  证据索引", 1)
    add_table(doc, ["证据", "位置", "用途"], [["自动化结果", "evidence/verification.json", "记录时间、版本、指标和样本"], ["系统设计 PDF", "01-system-design.pdf", "总体架构和技术边界"], ["源码包", "source/汽车直播智能体源码.zip", "交付和复现"], ["现场手册", "07-现场演示操作手册.md", "答辩演示顺序"]], [2400, 3600, 3360])
    add_sources(doc, ["GPT-SoVITS 官方中文说明：https://github.com/RVC-Boss/GPT-SoVITS/blob/main/docs/cn/README.md", "参赛题目文件：C:/Users/seele/Desktop/6.（海云捷迅）2026年第二届重庆市AI大模型创新应用大赛企业出题.docx"])
    return doc
...
async function main(){ const p=Presentation.create({slideSize:{width:1280,height:720}});
  ...
  s=p.slides.add(); s.background.fill="#FFFFFF"; title(s,"04 / RAG","答案来自证据单元，而不是来自猜测","清洗、切分、索引、版本和来源一起进入数据库。",5); const ys=[255,345,435,525], steps=["文件解析","清洗分段","结构化入库","混合检索"]; const desc=["PDF 页面 / DOCX 表格 / TXT 行序","标题、字段、标点和长度规则","车型、年款、版本、页码、哈希向量","词法命中 + 稳定向量 + 字段优先"]; for(let i=0;i<4;i++){box(s,88,ys[i],250,62,"#F7F9FB"); tx(s,steps[i],112,ys[i]+18,190,24,18,"#17212B",true); tx(s,desc[i],390,ys[i]+18,690,24,17,"#5C6873"); if(i<3) rule(s,213,ys[i]+62,0,"#135D9A",2);} box(s,880,240,300,170,"#EAF6F4","#B8DED8"); tx(s,"输出",912,270,120,24,16,"#1E8A9E",true); tx(s,"答案\n置信度\n来源片段",912,308,220,90,23,"#17212B",true); tx(s,"40/40 · 100%",88,620,260,32,26,"#135D9A",true); tx(s,"检索与最终问答均达标",360,624,400,24,18,"#5C6873");
  ...
  s=p.slides.add(); s.background.fill="#FFFFFF"; title(s,"09 / 指标","每个硬指标都对应一份可复现证据","不只报结果，也说明样本、目标和验证入口。",10); const rows=[["检索准确率","40/40 · 100%","目标 >=85%"],["问答准确率","40/40 · 100%","目标 >=85%"],["TTS 首音频","平均 2323 ms","目标 <=3000 ms"],["单元测试","35/35 passed","无失败"]]; for(let i=0;i<4;i++){const y=250+i*72; box(s,90,y,1100,54,i%2?"#F7F9FB":"#EEF3F7"); tx(s,rows[i][0],120,y+15,280,24,18,"#17212B",true); tx(s,rows[i][1],520,y+15,300,24,19,"#135D9A",true); tx(s,rows[i][2],900,y+15,220,24,16,"#5C6873");} tx(s,"证据文件：evidence/verification.json  ·  复现命令：verify.ps1",90,590,900,24,17,"#5C6873");
