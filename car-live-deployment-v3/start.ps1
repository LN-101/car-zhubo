...
if(-not (($gptRequired | Where-Object { -not (Test-Path $_) }).Count -eq 0)){
  throw 'GPT-SoVITS v2ProPlus weights are incomplete; check s1v3, s2Gv2ProPlus, s2Dv2Pro, and the eres2net speaker encoder.'
}
if((Test-Path $gptPython) -and (Test-Path $gptConfig) -and -not (Assert-PortOwner 9880 $gptPython 'api_v2.py' 'GPT-SoVITS')){
