...
// ⏱ Click-to-first-audio timer shown next to the transport controls.
...
const sentenceList = text => {
  ...
  // the server-side broadcast voice sound clipped and mechanical.
  const units = [];
...
// Mirrors the backend's zhubo-style broadcast segmentation so the 播报切分
// preview, the 实时话术 rows and what actually plays all agree: split at
// natural punctuation, group the first two segments within 16 characters and
// later ones within 32.
function broadcastUnits(text) {
  const normalized = normalizeSpeechText(text);
  if (!normalized) return [];
  const phrases = normalized.match(/.+?(?:[。！？!?；;，,\n]+|(?<!\d)\.(?!\d)|$)/gs)?.map(x => x.trim()).filter(Boolean) || [];
  if (!phrases.length) return [normalized];
  const units = [];
  let pending = '';
  for (const phrase of phrases) {
    const target = units.length < 2 ? 16 : 32;
    if (pending && pending.length + phrase.length > target) { units.push(pending); pending = ''; }
    pending += phrase;
  }
...
function renderScriptUnits() {
  try {
    ...
    const units = broadcastUnits(script.value);
...
function normalizeSpeechText(input) {
  ...
  text = text.replace(/[:：]/g, '，');
  // Remove layout spaces before queue splitting. The broadcasting voice would
  // otherwise treat spaces around model names and quantities as extra pauses.
...
async function studio() {
  ...
  try {
    ...
    const initializeUI = () => {
      ...
      safeClick('#itemEdit', itemEdit);
      safeClick('#itemAdd', itemAddItem);
      safeClick('#itemDelete', itemDeleteItem);
      safeClick('#ask', ask);
      ...
      ['speed','volume','pitch','intensity'].forEach(id => {
        const input = $('#' + id);
    ...
    <div class="avatar-workspace">
      ...
      <section class="avatar-controls">
        <section class="panel"><div class="panel-head"><h2>播报控制</h2><span id="ttsMode">${esc(tts.provider_label || '语音服务')}</span></div><div class="panel-body">
          ...
          <table class="item-table"><thead><tr><th>编号</th><th>状态</th><th>话术</th></tr></thead><tbody id="itemRows"></tbody></table>
          <div class="controls item-controls"><label>编号 <input id="itemId" type="number" min="1" step="1"></label><input id="itemText" class="question" placeholder="修改 / 新增话术"><label>新增到此编号之后 <input id="itemAfter" type="number" min="0" step="1" value="0"></label><button class="btn secondary" id="itemEdit">修改</button><button class="btn secondary" id="itemAdd">新增</button><button class="btn secondary" id="itemDelete">删除</button></div><p id="itemHint" class="hint"></p><p id="itemState" class="hint"></p>
        </div></section>
...
function updateOutputs() {
  ...
  if (intensity) safeSet('#intensityOut', 'textContent', Math.round(Number(intensity.value) * 100) + '%');
...
// ---- Live script items (backend-authoritative) ----------------------------
// The session item list is the single source of truth for what the broadcast
// reads; the console renders it, edits it through the item API, and the
// playback engine tags every synthesis request with its item id and revision.
function applySessionItems(data) {
  session = data;
  sessionItems = Array.isArray(data?.items) ? data.items.map(item => ({...item})) : [];
  renderItemTable();
  if (data?.version !== undefined) safeSet('#version', 'textContent', '会话版本 v' + data.version);
}

// Item-level editing needs per-item streaming. Batched providers
// (Zhubo/IndexTTS2) share one synthesis unit across several items, so the
// console disables item operations there and keeps the whole-script re-sync.
function updateItemControls() {
  const perItem = providerStreamsPerItem();
  const selected = sessionItems.find(item => item.id === Number(document.querySelector('#itemId')?.value || 0));
  const locked = !perItem || (selected ? selected.status === '已播放' : false);
  const editBtn = document.querySelector('#itemEdit');
  const addBtn = document.querySelector('#itemAdd');
  const delBtn = document.querySelector('#itemDelete');
  if (editBtn) editBtn.disabled = locked;
  if (delBtn) delBtn.disabled = locked;
  if (addBtn) addBtn.disabled = !perItem;
  safeSet('#itemState', 'textContent', sessionItems.length
    ? `${sessionItems.length} 条${perItem ? '' : ' · 当前引擎不支持条目级改稿，请用“重新同步脚本”'}`
    : '未开始');
}

function providerStreamsPerItem() {
  // Zhubo already pipelines phrase by phrase inside its own engine. Requesting
  // one item at a time streams the first short phrase as soon as it exists,
  // instead of waiting for a whole multi-sentence batch to finish synthesizing.
  return (ttsStatus?.provider || '') === 'zhubo';
}

function setItemHint(message) {
  safeSet('#itemHint', 'textContent', message || '');
}
...
function renderItemTable() {
  ...
  body.innerHTML = sessionItems.length
    ? sessionItems.map(item => `<tr data-id="${item.id}" class="${itemStatusClass(item.status)}"><td>${item.id}</td><td>${esc(item.status || '待播')}</td><td>${esc(item.text)}</td></tr>`).join('')
    : '<tr><td colspan="3">暂无话术</td></tr>';
  body.querySelectorAll('tr[data-id]').forEach(row => {
    row.onclick = () => {
      const item = sessionItems.find(candidate => String(candidate.id) === row.dataset.id);
      if (!item) return;
...
function saveLiveProgress(itemId, status = 'playing') {
  if (!session) return;
  api('/live/sessions/' + session.id + '/progress', {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({current_item_id:itemId, status})})
    .then(data => applySessionItems(data))
    .catch(() => {});
}

function stopSourcesFromIndex(run, fromIndex) {
  const now = audioCtx?.currentTime || 0;
  for (const source of [...run.sources]) {
    if ((source.liveIndex ?? 0) >= fromIndex) {
  ...
  run.scheduledUntil = Math.max(now + STREAM_LEAD_SECONDS, ...[...run.sources].map(source => source.liveEnd || 0));
}

function isItemRun(run) {
  return !!(run && run.items && run.items.length);
}

// Batched providers (Zhubo/IndexTTS2) tag every source with the whole
// batch range, so an item inside the audible batch cannot be cut alone. Map an
// item index onto the start of the covering batch; per-item runs (zhubo) have
// a one-item range, so this is exactly the item itself.
function batchStartFor(run, index) {
  if (run.currentBatchEnd > run.currentBatchStart && run.currentBatchStart <= index && index < run.currentBatchEnd) {
    return run.currentBatchStart;
  }
  const covering = [...run.sources].find(source =>
    (source.liveIndex ?? 0) <= index && index <= (source.liveEndIndex ?? source.liveIndex ?? 0));
  return covering ? (covering.liveIndex ?? index) : index;
}

// The index of the item the listener can currently hear. Generation runs
// ahead of playback, so this is the boundary that decides where an inserted
// item may go without interrupting (or being re-ordered after) audible audio.
function audibleRunIndex(run) {
  const now = audioCtx?.currentTime || 0;
  const audible = [...run.sources]
    .filter(source => (source.liveStart ?? 0) <= now && (source.liveEnd ?? 0) > now)
    .map(source => source.liveIndex ?? 0);
  if (audible.length) return Math.max(...audible);
  const upcoming = [...run.sources]
    .filter(source => (source.liveStart ?? 0) > now)
    .map(source => source.liveIndex ?? 0);
  if (upcoming.length) return Math.min(...upcoming);
  return Math.max(0, run.currentIndex ?? 0);
}

// Report the item that is actually audible (not the item being generated) so
// the backend only marks finished items as played; generation runs ahead of
// playback and would otherwise 409 edits on audio the listener still hears.
function reportAudibleProgress(run) {
  if (!isItemRun(run)) return;
  const index = Math.min(audibleRunIndex(run), run.items.length - 1);
  const item = run.items[index];
  if (item && item.id !== run.reportedItemId) {
    run.reportedItemId = item.id;
    saveLiveProgress(item.id, 'playing');
  }
}

// Rebuild the playback queue from the current item list and resume from
// `fromIndex`. When that index is the item already playing (or is still being
// synthesized), the in-flight generation is invalidated so the revised text is
// synthesized and played from its start.
function restartRunFrom(run, fromIndex, message) {
  if (!run || run.stopped || liveRun !== run) return;
  const cut = batchStartFor(run, fromIndex);
  const interrupted = cut <= (run.currentIndex ?? 0);
  const coversIndex = run.inFlight > 0 && run.currentBatchStart <= cut && cut < run.currentBatchEnd;
  if (interrupted || coversIndex) {
    run.revision += 1;
    run.abortController?.abort();
    // A stale in-flight stream restarts from this flag in its finally block,
    // including after a paused resume; only a run with nothing in flight owns
    // the restart itself and must clear the flag.
    run.pendingRestartAt = run.inFlight > 0 ? cut : null;
  } else {
    run.pendingRestartAt = null;
  }
  if (interrupted) stopSourcesFromIndex(run, cut);
  else discardUnplayedSources(run, cut);
  run.q = run.items.map(item => item.text);
  run.nextIndex = Math.min(run.nextIndex, cut);
  if (!run.paused && run.nextIndex < run.q.length && !run.inFlight) void streamGptUnit(run);
  if (message) setState(message);
}

function onItemEdited(id) {
  const run = liveRun;
  if (!run || run.stopped || run.mode !== 'gpt-sovits') return;
  const runIndex = run.items.findIndex(item => item.id === id);
  const item = sessionItems.find(candidate => candidate.id === id);
  if (runIndex < 0 || !item) return;
  run.items[runIndex].text = item.text;
  run.items[runIndex].revision = item.revision;
  restartRunFrom(run, runIndex, `已修改第 ${runIndex + 1} 条；从该条重新播报`);
}

function onItemAdded(id) {
  const run = liveRun;
  if (!run || run.stopped || run.mode !== 'gpt-sovits' || !isItemRun(run)) return;
  const listIndex = sessionItems.findIndex(item => item.id === id);
  if (listIndex < 0) return;
  const item = sessionItems[listIndex];
  // Anchor on the item the new one follows so the inserted position stays
  // correct even after run.items and sessionItems have diverged, and never let
  // an item inserted before the current position land in the played region.
  const predecessor = listIndex > 0 ? sessionItems[listIndex - 1] : null;
  const predecessorRunIndex = predecessor ? run.items.findIndex(entry => entry.id === predecessor.id) : -1;
  const desired = predecessorRunIndex >= 0 ? predecessorRunIndex + 1 : run.items.length;
  // Clamp against the audible item, not the generation head: an item inserted
  // after a 编号 must play right after that item, and an insert in the already
  // played region plays next instead of interrupting the current audio.
  const insertAt = Math.max(desired, audibleRunIndex(run) + 1);
  run.items.splice(insertAt, 0, {id:item.id, revision:item.revision, text:item.text});
  restartRunFrom(run, insertAt, `已新增第 ${insertAt + 1} 条；按顺序播出`);
}
...
function onItemDeleted(id) {
  ...
  run.items.splice(runIndex, 1);
  restartRunFrom(run, runIndex, `已删除第 ${runIndex + 1} 条；继续下一条`);
}

function onScriptSynced() {
  const run = liveRun;
  if (!run || run.stopped || run.mode !== 'gpt-sovits' || !isItemRun(run)) return;
  const next = sessionItems.map(item => ({id:item.id, revision:item.revision, text:item.text}));
  let changedAt = 0;
  while (changedAt < run.items.length && changedAt < next.length &&
         run.items[changedAt].id === next[changedAt].id &&
         run.items[changedAt].revision === next[changedAt].revision) changedAt += 1;
  if (changedAt >= run.items.length && changedAt >= next.length) { setState('稿件已同步'); return; }
  run.items = next;
  restartRunFrom(run, changedAt, `稿件已同步；从第 ${changedAt + 1} 条使用新稿`);
}

// The server-side broadcast returns stable phrase-sized PCM fragments. The
// browser queues each fragment in one AudioContext without adding synthetic pauses.
...
// The first request is intentionally smaller. The server must tokenize and
...
function stopLive({suspend = false, clearTimer = true} = {}) {
  if (clearTimer) stopLatencyTimer();
  lipSync.clear();
  const run = liveRun;
...
// 首音频计时：从点击播报（含就绪等待）到第一个可听 PCM 片段的端到端耗时。
...
// Includes readiness waiting, session creation, PCM arrival and output buffering.
...
function createLiveRun(mode, q, startIndex = 0, items = []) {
  return {
    requestedAt:performance.now(),
    mode, q, items:(items || []).map(item => ({...item})), nextIndex:startIndex, currentIndex:startIndex, stopped:false, paused:false,
    failed:false,
    ...
    currentItemId:null, currentItemRevision:null, reportedItemId:null,
...
function finishRunIfDrained(run) {
  ...
  if (isItemRun(run)) saveLiveProgress(null, 'completed');
  window.CarLiveFeatures?.flushPlayback();
...
function schedulePcm(run, bytes, unitIndex, unitEnd = unitIndex + 1) {
  ...
  const scheduleAvailable = flush => {
    ...
    const minimum=Math.ceil(sampleRate*.12);
    ...
    while (wholeFramesForQueue(run, bytesPerFrame) > 0) {
      ...
      source.onended = () => { run.sources.delete(source); sourceGain.disconnect(); if (activeSource === source) activeSource = null; reportAudibleProgress(run); finishRunIfDrained(run); };
      source.start(when);
...
async function streamGptUnit(run) {
  ...
  const perItem = providerStreamsPerItem();
  const batchSize = perItem ? 1 : (index === 0 ? STREAM_FIRST_BATCH_UNITS : STREAM_BATCH_UNITS);
  ...
  run.currentItemId = run.items[index]?.id ?? null;
  run.currentItemRevision = run.items[index]?.revision ?? null;
  run.inFlight += 1;
  ...
  try {
    const response = await fetch(API + '/tts/stream', {method:'POST', headers:{'Content-Type':'application/json'}, body:ttsBody(batchText, true, !perItem, run.currentItemId, run.currentItemRevision), signal:controller.signal});
...
async function startGptRun(q, startIndex = 0, items = []) {
  // Keep the click-to-first-audio timer running across the run hand-off.
  stopLive({ clearTimer: false });
  const run = createLiveRun('gpt-sovits', q, startIndex, items);
...
async function resumeLive(run) {
  if (run.mode === 'browser') { run.paused = false; run.pendingRestartAt = null; speechSynthesis.resume(); setState('浏览器语音继续播报'); return; }
  ...
  // When a stale stream is still in flight it owns the restart via
  // pendingRestartAt; only clear the flag when this call starts the work.
  if (!run.inFlight) { run.pendingRestartAt = null; void streamGptUnit(run); }
...
async function play() {
  ...
  if (!broadcastUnits(scriptText).length) return setState('请先填写直播稿');
  const intent=++audioIntent;
  playPending = intent;
  updatePlaybackControls();
  ...
  try {
    ...
    // A fresh broadcast uses the current textarea. Re-sync it into the existing
    // session, and start a new session when the script changed or a previous run
    // already finished so stale 已播放 flags cannot block the new broadcast.
    if (session && (session.script !== scriptText || sessionItems.some(item => item.status === '已播放'))) {
      session = null; sessionItems = [];
    }
    try { await ensure(); } catch (error) { setState('无法创建直播会话：' + error.message); return; }
    if (intent !== audioIntent) return;
    const items = sessionItems.map(item => ({id:item.id, revision:item.revision, text:item.text}));
    const q = items.length ? items.map(item => item.text) : broadcastUnits(scriptText);
    if (!q.length) return setState('请先填写直播稿');
    renderItemTable();
    if (ttsMode === 'gpt-sovits') await startGptRun(q, 0, items); else startBrowserRun(q);
  } catch (error) {
...
async function revise() {
  ...
  if (!text) return setState('改稿内容不能为空');
  const loadingId = activeViewLoading;
  const intent = audioIntent;
  ...
  try {
    ...
    // The backend re-syncs the textbox against the item list: played items are
    // preserved, and a change that would touch one is rejected there.
    const data = await api('/live/sessions/' + saved.id + '/script', {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({script:text, sentence:0})});
    if (loadingId !== activeViewLoading || intent !== audioIntent || session?.id !== saved.id) return;
    applySessionItems(data);
    window.CarLiveFeatures?.revision();
    if (!liveRun || liveRun.stopped) { setState('稿件已同步；点击“开始播报”后使用新稿'); return; }
    onScriptSynced();
  } catch (error) {
    if (loadingId === activeViewLoading && intent === audioIntent && error.name !== 'AbortError') setState('改稿失败：' + error.message);
  } finally {
...
async function itemEdit() {
  ...
  if (!providerStreamsPerItem()) return setItemHint('当前语音引擎不支持条目级改稿；请使用“重新同步脚本”。');
  if (!id || !text) return setItemHint('请先选择条目并填写修改后的话术');
  try {
    await ensure();
    applySessionItems(await api(`/live/sessions/${session.id}/items/${id}`, {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({text})}));
    onItemEdited(id);
    window.CarLiveFeatures?.revision();
    setItemHint('已修改话术');
  } catch (error) { setItemHint(error.message); }
}

async function itemAddItem() {
  const text = ($('#itemText')?.value || '').trim();
  if (!providerStreamsPerItem()) return setItemHint('当前语音引擎不支持条目级改稿；请使用“重新同步脚本”。');
  if (!text) return setItemHint('请填写新增话术');
  try {
    await ensure();
    const before = new Set(sessionItems.map(item => item.id));
    const after = Number($('#itemAfter')?.value || 0);
    const data = await api(`/live/sessions/${session.id}/items`, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({text, after_id: after > 0 ? after : null})});
    applySessionItems(data);
    const added = sessionItems.find(item => !before.has(item.id));
    if (added) onItemAdded(added.id);
    window.CarLiveFeatures?.revision();
    setItemHint('已新增话术');
  } catch (error) { setItemHint(error.message); }
}

async function itemDeleteItem() {
  const id = Number($('#itemId')?.value || 0);
  if (!providerStreamsPerItem()) return setItemHint('当前语音引擎不支持条目级改稿；请使用“重新同步脚本”。');
  if (!id) return setItemHint('请先选择要删除的条目');
  try {
    await ensure();
    applySessionItems(await api(`/live/sessions/${session.id}/items/${id}`, {method:'DELETE'}));
    onItemDeleted(id);
    window.CarLiveFeatures?.revision();
    setItemHint('已删除话术');
  } catch (error) { setItemHint(error.message); }
...
function ttsBody(text, unitized = false, streamBatch = false, itemId = null, itemRevision = null) {
  ...
  return JSON.stringify({text:normalizeSpeechText(text), unitized,
    ...
    speed_factor:Number(value('#speed', 1)),
    delivery:'natural',
    tone: value('#tone', 'neutral'),
    intensity: Number(value('#intensity', 0.3)),
    // Browser gain controls live volume; avoid applying it twice in Zhubo PCM.
    volume: 1});
}
