// Live-console workflow regression tests.
//
// Adapted from the upstream car-live-deployment suite (frontend/tests/live-workflow.test.cjs)
// to this fork: the console keeps the whole-script re-sync (改稿失败) message instead of
// the upstream queue-rewrite wording, and the tests covering GPT-SoVITS-only readiness
// gating (`live_path_warmed`) and first-phrase priming were left out because this fork
// drives the Zhubo broadcast service, which has no equivalent endpoint.
...
test('failed revision keeps the current audio queue and restores the save button', async () => {
  ...
  assert.match(h.nodes['#playstate'].textContent, /改稿失败/);
});
