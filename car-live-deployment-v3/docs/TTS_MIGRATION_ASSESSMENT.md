...
> 2026-09-10 更新：本节的默认值判断已过期。语音引擎默认值早已从 GPT-SoVITS 切到 IndexTTS-2.5（`backend/.env` 与 `backend/.env.example` 均为 `TTS_PROVIDER=idextts2`，`backend/app/config.py` 的代码默认值也已对齐）。截至该日期，运行中的 IndexTTS2 实例实际加载的还是 2.0 权重，切换未完成；核查与整改见 [IndexTTS-2.5 切换核查报告](TTS_INDEX_TTS25_SWITCH_AUDIT.md)。本文以下关于 AstraTTS / C# + ONNX 的评估仍然有效。

本文评估时的默认值：

```text
TTS_PROVIDER=gpt-sovits   # 已被 IndexTTS-2.5 取代
```
...
- 当前项目支持 `base` 和 `xilian` 两个 GPT-SoVITS 模型 profile，并按音色动态切换。
...
- 当前工作区的 Xilian SoVITS 权重文件还不能被正常反序列化，因此不能把 Xilian 路由视为已完成的 ONNX 迁移。
...
| Xilian 音色 | 高 | 当前权重文件本身无法正常读取，不能迁移验收 |
...
- 语音引擎默认值为 `TTS_PROVIDER=idextts2`（IndexTTS-2.5），GPT-SoVITS 通过 `TTS_PROVIDER=gpt-sovits` 仍可完整切回。
- 不启动未完成的 C# 服务，AstraTTS 配置字段保持为独立实验入口。
- 本次生成的 ONNX 文件只用于转换和结构验证，不作为已验收的生产模型。
- `INDEX_TTS2_CONFIG` / `INDEX_TTS2_MODEL_DIR` 必须指向 2.5 权重；指向 2.0 会静默退回旧引擎，`start.ps1` 会就此发出警告。
